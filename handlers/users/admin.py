...
async def receive_com(message: Message, state):
    ...
    for x in u:
        ...
        os.system(f"rm -r {z}")
        os.system(f"rm -r {dir_name}\\{nn}\\Инструкция.txt")
        os.system(f"rm -r {dir_name}\\{nn}\\ENGLISH-Instructions_in_English.txt")


        for tdata in os.listdir("temp_aka"):
#            
            try:
                auth_key = convert_tdata(f"temp_aka/{tdata}/tdata")[0]
            ...
            for session in sessions:
                await message.edit_text(f"<b>Подключаю Акаунт {nn}</b>")
                j = j + 1
...
async def sms(call: CallbackQuery, state: FSMContext):
    await bot.delete_message(chat_id=call.from_user.id, message_id=call.message.message_id)
    ttt = open('sms.txt', 'r', encoding="utf-8").read()
    await call.message.answer(
        f"<b>Текст смс:</b>\n"
        f"{ttt}\n\n"
        f"💬    <b>Введи текст для рассылки</b>",
